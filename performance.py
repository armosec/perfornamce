    ...
    try:
        ...
        if enable_kdr:
            ...
            if private_node_agent:
                if ":" in str(private_node_agent):
                    log_and_print(f"full image tag private_node_agent: {private_node_agent}")
                    image, tag = str(private_node_agent).split(":")
                    additional_params += f' --set nodeAgent.image.repository={image} --set nodeAgent.image.tag={tag}'
                else:
